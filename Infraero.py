...
class FlightDataHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'span':
            dic = dict(attrs)
            if dic.has_key('id'):
                if dic.get('id').startswith('grd_voos_ctl') or \
                   (dic.get('id') == 'lbl_data_pagedate'):
...
class Harvester(object):
    ...
    def request_flight(self, airport, flight):
        """Returns information about an specific flight at an airport"""
        self.pagemax = 1
    ...
    def request_airport(self, airport):
        """Returns information about all flights at an airport"""
        self.pagemax = 1
...
class Formater(object):
    ...
    def __init__(self, flights):
        """Constructor method receives the flights information"""
        self.flights = flights

    #pylint: disable=no-self-use
    def print_table_row(self, flight, head):
        """Prints a flight row with flight information and
        a boolean that set the row as a head row"""
        if head:
            headertype = 'h'
            index = 0
        else:
        ...
        print '<tr>'
        for data in flight:
            print '<t' + headertype + ' style="color: ' + color + '">'
            print data[index]
            print '</t' + headertype + '>'
        print '</tr>'

    def print_table(self, tableid):
        """Prints a table with the specified id"""
        first = True
